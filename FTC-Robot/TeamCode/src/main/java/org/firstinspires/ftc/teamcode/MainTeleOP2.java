 ...
 * Sticks cancel navigation. On release, target editing starts from current pose.
 * Neutral sticks: D-pad edits target 1 inch in the CURRENT robot frame.
 * X / B: edit target heading CCW / CW by 5 degrees. Y: turn, then navigate.
 * LB: cancel + sync target. Back: stop, settle, then reset pose/calibrate IMU.
...
public class MainTeleOP2 extends OpMode {
    ...
    // TeleOp-only policy: manual driving discards the old navigation target.
    // Programmatic Odometry2.cancelNavigation() still preserves targets.
    private boolean manualTargetsNeedSync;
    ...
    public void loop() {
        ...
        if (driving) {
            odometry.cancelPendingReset();
            manualTargetsNeedSync = true;
        } else if (manualTargetsNeedSync && odometry.isReady()) {
            // Sync once, using the current cached pose, BEFORE processing X/B/Y.
            // If odometry is unavailable, defer until a valid reference returns.
    ...
    private void sendTelemetry(boolean force) {
        ...
        telemetry.addLine("Y: GO | sticks: cancel + rebase target on release | LB: sync");
